...
class ReducedPhoto {
    constructor(country, region, lng, lat, title) {
...
var getKmlFoldersFromPhotos = (photos) => {
    var folders = [];
    for (var countryName in photos) {
        if (photos.hasOwnProperty(countryName)) {
            var placemarkKmlNodes = [];
            photos[countryName].photos.forEach((photo) => {
...
var getKmlFolder = (folderName, placemarks) => {
    return `		<Folder>
...
var getPlacemarkKmlNode = (lat, lng, title) => {
    return `			<Placemark>
				<name>${title}</name>
				<Point>
...
var photosByCountry = photos
    ...
    if (previousPhoto[country]) {
        previousPhoto[country].addPhoto(currentPhoto);
    }
    else {
